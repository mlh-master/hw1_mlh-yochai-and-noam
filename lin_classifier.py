...
def odds_ratio(w, X, selected_feat='LB'):
    ...
    # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
    x_t = np.transpose(X)
    w_t = np.transpose(w[0])
    selectet_feature_idx = X.columns.get_loc(selected_feat)
    all_odds = 1 / (np.exp(-(w_t.dot(x_t))))
    odds = np.median(all_odds)
    odd_ratio = np.exp(w_t[selectet_feature_idx])
    # --------------------------------------------------------------------------
